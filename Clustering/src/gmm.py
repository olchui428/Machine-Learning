...
class GMM():
    ...
    def _m_step(self, features, assignments):
        ...
        new_cov = np.zeros(self.means.shape)
        new_weights = np.zeros(self.n_clusters)
        ...
        for j in range(self.n_clusters):
            ...
            big_gamma = np.sum(assignments[:,j])
            for i in range(features.shape[0]):
                mean_tot += features[i] * assignments[i,j]
                sig_tot += assignments[i,j] * ((features[i] - means[j])**2)

            mu = mean_tot / big_gamma
            sig = sig_tot / big_gamma
            w = big_gamma / features.shape[0]
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        cur_mean = self.means[k_idx]
        cur_covar = self.covariances[k_idx]
        cur_mixweight = self.mixing_weights[k_idx]

        likelihood = multivariate_normal.logpdf(features, mean=cur_mean, cov=cur_covar)
        loglikelihood = np.log(cur_mixweight) + likelihood
